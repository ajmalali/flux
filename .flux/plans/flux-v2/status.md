...
Updated: 2026-09-03, later (**loop phase 01 — `flux ledger` — BUILT, gated, wrapped.** The sensor the whole `loop` effort is judged by now exists: `flux ledger [--since] [--fleet] [--json]` in `bin/flux` mines the on-disk transcripts and prints the read-out's targets table, per cycle, budget-clipped, with a fleet view + `meta-tax` line. 23 new tests + a checked-in 20-record golden fixture, **207 green** (was 184). **AC-1/2/3/4 met on every reproducible metric** — rpi TOTAL reproduces the read-out exactly: 16 substantive sessions, ctx p50 87k, 0 sessions >150, 11/16 wrapped, $193, first edit after 11 calls. Two documented deviations, no work left: the gate ratio prints 16/25 not the read-out's "24/35" (that number was an **unsaved ad-hoc grep** counting cross-repo greps + backend pytest — unreproducible, direction preserved), and ledger medians exclude void/minor sessions per AC-2 where the read-out took medians over all sessions (so bash 21k vs 18.6k, etc. — reproduces the read-out's numbers exactly over the all-sessions set). Fleet runs in 1.5 s over 330 MB and clips to 754 B; benchmark clones under `.flux-bench/runs/` excluded, repos resolved by transcript `cwd`. Evidence side-by-side: `.flux/plans/loop/01-flux-ledger.status.md`. **NEXT: phase 02 status diet — its plan is not written yet;** `/flux:plan .flux/plans/loop/02-status-diet.md`, then cut this 106 KB status.md to ≤8 KB live state (flux-repo ctx p50 94k → <75k).)

Updated: 2026-09-03 (**first field read-out of flux v2 on real work — `.flux/analysis/2026-09-03-field-readout.md`.** Every transcript for kiosk / rpi-rfm69 / broadcast / radiator / flux-itself since 2026-08-20 was mined (44 adopting-repo sessions, $867 API-equivalent; flux spent $415 on itself). **rpi-rfm69 is the shape flux was built for and met or grazed every target** (ctx p50 87k, 0 sessions >150 req, 11/16 wrapped, ~$26/phase, first edit after 11 calls vs 20 in kiosk-before; 3/3 audits landed real hits, 1 blocking — pre-registration **C2 met**). **radiator is the failure shape**: wrap ran in 3/13 sessions, `next` was identical for six sessions running, five sessions blew past 150 requests ($487 of $645), and state re-bloated into a 21 KB status.md + 46 KB plan read whole each start — the flux repo does the same with its own 106 KB status.md. Cross-cutting: the model bypasses the filtered gate (raw 35 vs `flux check` 24 in rpi, 59 vs 9 in radiator; `flux run` 0 uses), rediscovers the CLI each session (`flux --help` ×6, skill files cat-ed from the plugin cache ×4), stale keys ride unflagged (rpi `blocker` 5 days past resolution), `routing` is dead (17/18 writes `design`, all sessions Opus), and 9 of 14 skills had zero invocations. Thirteen actions filed, then — same session — **ADR 0003 (flux measures itself) accepted and the `loop` effort planned**: `.flux/plans/loop/00-roadmap.md` (8 phases, each with its claim) and `01-flux-ledger.md` written in full. **No code changed; 184 tests green.**)
...
- [ ] **NEXT: build the loop — `.flux/plans/loop/` (ADR 0003, accepted 2026-09-03,
      unbuilt).** Roadmap `00-roadmap.md` maps every read-out action to eight phases
      with the claim each is judged on. **Start with `/flux:apply
      .flux/plans/loop/01-flux-ledger.md`** (`flux ledger`: transcripts → targets
      table, budgeted, tested; oracle numbers in the plan's AC-1). Then 02 status diet
      (this file → ≤ 8 KB), 03 `flux log` + pack footer, 04 guard/age/seal hooks,
      05 claims + cycle line, 06 handoff inline, 07 deletions (user decides), 08
      optional. One phase per session; wrap every session.

- [ ] **Field read-out actions — 2026-09-03** *(superseded by the loop roadmap above;
      kept as the evidence index).* Source and evidence:
