...
class _Repository(object):
    ...
    """Datetimes that may legitimately be absent; ``null`` on disk, ``None`` in memory."""
    ...
    def _to_row(self, obj):
        ...
        for field_name in self.optional_date_fields:
            value = row.get(field_name)
            row[field_name] = to_iso(value) if value is not None else None
        return row
    ...
    def _from_row(self, row):
        ...
        for field_name in self.optional_date_fields:
            ...
            data[field_name] = from_iso(value) if value is not None else None
...
class BookingRepository(_Repository):
    ...
    optional_date_fields = ("expires_at",)
