...
- AC-6: /plan writes each settled section into specs/<slug>/spec-draft.md in the round it settles, and gives one notice at 200k and one at 350k against `context_tokens` in `.flux/session.json` — a count read from a named source, never estimated, and reported as unavailable when that field is null — without stopping the run on its own; the run ends with an approved spec.md, or — when the user calls a pause or context pressure forces one — a draft plus handoff.
